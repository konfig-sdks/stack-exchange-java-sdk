...
|------------- | ------------- | -------------|
| [**getUserFavorites**](FavoriteApi.md#getUserFavorites) | **GET** /me/favorites |  |


<a name="getUserFavorites"></a>
# **getUserFavorites**
> List&lt;QuestionsInner&gt; getUserFavorites(site).order(order).max(max).min(min).sort(sort).fromdate(fromdate).todate(todate).pagesize(pagesize).page(page).filter(filter).paramCallback(paramCallback).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      ...
      System.err.println("Exception when calling FavoriteApi#getUserFavorites");
