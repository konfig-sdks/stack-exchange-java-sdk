...
|------------- | ------------- | -------------|
| [**userCommentsList**](MentionApi.md#userCommentsList) | **GET** /me/mentioned |  |


<a name="userCommentsList"></a>
# **userCommentsList**
> List&lt;CommentsInner&gt; userCommentsList(site).order(order).max(max).min(min).sort(sort).fromdate(fromdate).todate(todate).pagesize(pagesize).page(page).filter(filter).paramCallback(paramCallback).execute();
...
public class Example {
  public static void main(String[] args) {
    ...
    try {
      ...
      System.err.println("Exception when calling MentionApi#userCommentsList");
